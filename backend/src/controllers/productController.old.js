...
export async function getProductSuppliers(request, reply) {
  try {
    ...
    const product = await Product.findOne({
      ...
      .select('manufacturerRef name supplierInfos')
      .lean();
...
export async function getProducts(request, reply) {
  try {
    ...
    // Add search filter (search in name, manufacturerRef, oemRefs, description)
    if (search) {
      filter.$or = [
...
export async function createProduct(request, reply) {
  try {
    ...
    // Check if manufacturerRef already exists for this company
    // Check for any product with this manufacturerRef that is not deleted (isDeleted !== true)
    ...
    if (existingProduct) {
      return reply.code(409).send({
        error: 'Product with this manufacturerRef already exists',
      });
    }
    ...
    if (error.code === 11000) {
      return reply.code(409).send({
        error:
          'Product with this manufacturerRef already exists for this company',
      });
    }
...
export async function updateProduct(request, reply) {
  try {
    ...
    // If manufacturerRef is being updated, check for duplicates
    if (manufacturerRef && manufacturerRef !== product.manufacturerRef) {
      const existingProduct = await Product.findOne({
